...
class TFSHooks(
    ...
    override fun postHook(body: Hook?, bugs: List<Int>): ResponseEntity<Any> {
        return try {
            when {
                ...
                else -> return ResponseEntity.status(HttpStatus.CREATED).body(saveHookToDatabase(body, null, null, "Bug state and sprint didn't change", null))
            }
            ...
            val actualIssueState = issueService.search("#$ytId", listOf("idReadable", "customFields(name,value(name))")).firstOrNull() ?: return ResponseEntity.status(HttpStatus.CREATED)
                .body(saveHookToDatabase(body, null, null, "Issue with id $ytId not found in YouTrack", null))
            val actualIssueFieldState = actualIssueState.unwrapFieldValue("State")
            if (actualIssueFieldState != "Направлена разработчику") return ResponseEntity.status(HttpStatus.CREATED)
                .body(saveHookToDatabase(body, actualIssueFieldState, null, "Issue with id $ytId is not on 3rd line", null))
            ...
            val inferredState = getInferredState(bugStates)
            ...
            if (body.isFieldChanged("System.State") && inferredState in arrayOf("Closed", "Proposed", "Resolved")) {
                fieldState = postCommand(ytId, "Состояние Открыта").body.toString()
            }
            when {
                inferredState !in arrayOf("Closed", "Resolved")
                        && !body.wasExcludedFromSprint()
                        && (body.isFieldChanged("System.State") || body.isFieldChanged("System.IterationPath"))
                -> {
                    fieldDetailedState = postCommand(ytId, "Детализированное состояние $inferredState").body.toString()
                }
                inferredState !in arrayOf("Closed", "Resolved")
                        && body.isFieldChanged("System.State")
                        && body.getFieldValue("Microsoft.VSTS.Common.ResolvedReason") == "Fixed"
                -> {
                    fieldDetailedState = postCommand(ytId, "Детализированное состояние Backlog проверки").body.toString()
                }
            }
    ...
    override fun mergeStates(devOpsBugStates: List<DevOpsBugState>, hook: Hook): List<DevOpsBugState> {

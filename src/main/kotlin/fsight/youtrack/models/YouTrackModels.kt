...
fun YouTrackActivity.toIssueHistoryRecord(idReadable: String): IssueHistoryRecord {
    ...
    val addedValue: Any? = when {
        ...
        !isMultiValue && type in listOf("state", "enum", "ownedField", "version") && constAdded is List<*> -> {
            (constAdded.firstOrNull() as? LinkedTreeMap<*, *>)?.get("name")
        }
    ...
    val removedValue: Any? = when {
        ...
        !isMultiValue && type in listOf("state", "enum", "ownedField", "version") && constRemoved is List<*> -> {
